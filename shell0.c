  ...
  while (!done) {
	...
	printf("NOP\n");
      ...
      printf("%s\n", arg);      
        ...
        //clear all 4 registers
        for (i=0; i<rmax; i++) {
            r[i] = 0;
        }
    } else if (strcmp(cmd, "ADD") == 0) {
        //add r[0] and r[1], put answer into r[0], move 0->r[3], r[3]->r[2], r[2]->r[1]
        r[0] = r[0] + r[1];
        r[1] = r[2];
        r[2] = r[3];
        r[3] = 0;
    } else if (strcmp(cmd, "HELP") == 0) {
        //print out short description of all valid commands
        printf("EXIT: exit from shell\n");
        printf("QUIT: exit from shell\n");
        printf("LOADI: load (immediate) arg into r[0] and push the other registers up\n");
        printf("PRTRS: print out the value of all the registers\n");
        printf("ECHO: echo arg\n");
        printf("CLRRS: clear all 4 registers\n");
        printf("ADD: r[0] + r[1], put answer into r[0], move 0->r[3], r[3]->r[2], r[2]->r[1]\n");
        printf("HELP: print out short description of all valid commands\n");
