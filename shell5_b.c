...
// are we currently reading from a file?
...
FILE *runfp = NULL;           // file pointer for run command
...
int main() {
    ...
    while (!done) {
        ...
        if (!readFromFile) {
            //prompt user for input and read input string
            printf("> ");
            fgets(cmd_input, SMAX, stdin);
        } else {
            fgets(cmd_input, SMAX, runfp);
            if (feof(runfp) == 0) {
                // strip off last \n if it was read in from file
                if (cmd_input[strlen(cmd_input) - 1] == '\n') {
                    cmd_input[strlen(cmd_input) - 1] = ' ';
                }

                // echo input so user can see what was read from file
                printf("RUN> %s\n", cmd_input);

            } else {

                // we ran off the end of the file, so  terminate run command and close the file
                readFromFile = false;
                fclose(runfp);

                // treat eof as if no command found, substitute NOP
                strcpy(cmd_input, "NOP");
                printf("RUN> EOF\n");
        ...
        if ((strcmp(cmd, "EXIT") == 0) || (strcmp(cmd, "QUIT") == 0)) {
            ...
            if (readFromFile) {
                // if exiting while reading commands from a file - close the file first!!
                fclose(runfp);
                printf("====================\n");
                printf("RUN - end reading from file\n");
                printf("====================\n");
            ...
            // start reading commands from file
            // file name in second argument (arg)

            if (readFromFile) {
                // not allowed to open a file for RUN while inside another RUN
                printf("RUN - RUN not allowed inside RUN -- NOP\n");
            } else {
                runfp = fopen(arg, "r");

                if (runfp != NULL) {
                    // we have a valid file to read from
                    printf("====================\n");
            ...
            printf("RUN arg: run commands from file 'arg'\n");
