...
int main () {
    ...
    while (!done) {
        ...
        if ((strcmp(cmd, "EXIT") == 0) || (strcmp(cmd, "QUIT") == 0)) {
            ...
            printf("SUB: subtracts r[1]-r[0], puts answer into r[0], scroll registers down\n");
            printf("MUL: multiplies r[1]*r[0], puts answer into r[0], scroll registers down\n");
            printf("DIV: divides r[1]/r[0], puts answer into r[0], scroll registers down\n");
            printf("REM: computes remainder of r[1] mod r[0], puts answer into r[0], scroll registers down\n");
            printf("PRTMS: print all memory locations\n");
            printf("LOADM: copy m[arg] to r[0], scroll other registers up\n");
            printf("STORM: move r[0] to m[arg], scroll other registers down\n");
            ...
            // move r[0] to m[arg]
            m[iarg] = r[0];
            // scroll other registers down (only valid for 0 <= arg <= 15)
            for (i = 1; i < rmax; i++) {
                r[i - 1] = r[i];
            }
            r[3] = 0;
